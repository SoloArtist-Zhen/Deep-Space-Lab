...
from deep_space_lab.halo.diff_correction import single_shoot_periodic, multi_shoot_periodic
...
OUT = "deep_space_lab/outputs/fast"
os.makedirs(OUT, exist_ok=True)

def halo_fast():
    mu = 0.0121505856
    extra0 = dict(c_srp=0.0, J2_e=0.0, J2_m=0.0, Re_bar=0.01, Rm_bar=0.003, sun_x=10.0)
    extraP = dict(c_srp=1.5e-6, J2_e=8e-4, J2_m=1.5e-4, Re_bar=0.01, Rm_bar=0.003, sun_x=10.0)
    ...
    fig_halo_family(amps, periods, energies)
    fig_monodromy_spectral(amps, rads, None)
    ...
    for i, fig in enumerate(plt.get_fignums()):
        plt.figure(fig).savefig(f"{OUT}/halo_fast_{i:02d}.png", dpi=160)
    plt.close('all')
...
def pareto_fast():
    ...
    obj = demo_objective_builder(mu, extra, start, target, T_bounds=(0.2,1.0))
    bounds=[(0.2,0.8), (-0.04,0.04),(-0.04,0.04),(-0.04,0.04), (0.2,0.8), (-0.04,0.04),(-0.04,0.04),(-0.04,0.04)]
    X, F, hv = nsga2(obj, bounds, pop=12, gen=8, pc=0.9, pm=0.2)
    ...
    for i, fig in enumerate(plt.get_fignums()):
        plt.figure(fig).savefig(f"{OUT}/pareto_fast_{i:02d}.png", dpi=160)
    plt.close('all')
...
def od_fast():
    ...
    ts, Y = propagate_with_stm(s0, 0.4, mu, extra, h0=2e-3, with_stm=False); traj=Y[:,:6]
    ...
    for i, fig in enumerate(plt.get_fignums()):
        plt.figure(fig).savefig(f"{OUT}/od_fast_{i:02d}.png", dpi=160)
    plt.close('all')

if __name__=="__main__":
    halo_fast(); pareto_fast(); od_fast()
    print("Fast figures saved to", OUT)
