...
def fig_monodromy_spectral(ampl, rad):
...
def fig_iter_curves(ms_hist, col_hist, t_ms, t_col):
    ...
    if ms_hist is not None: plt.semilogy(ms_hist, label="Multiple shooting")
    if col_hist is not None: plt.semilogy(col_hist, label="Collocation")
    plt.xlabel("Iteration"); plt.ylabel("Residual norm"); plt.title("Residual"); plt.grid(True); plt.legend(); plt.tight_layout()
...
def fig_pareto_3d(F):
    ...
    fig=plt.figure(figsize=(6,4.5)); ax=fig.add_subplot(111, projection='3d'); ax.scatter(F[:,0],F[:,1],F[:,2],s=20); ax.set_xlabel("ΔV"); ax.set_ylabel("TOF"); ax.set_zlabel("Miss/Robust"); ax.set_title("Pareto front"); plt.tight_layout()
...
def fig_srp_j2_compare(base_xy, pert_xy):
    plt.figure(figsize=(6,4)); plt.plot(base_xy[:,0], base_xy[:,1], label="CRTBP"); plt.plot(pert_xy[:,0], pert_xy[:,1], label="CRTBP+SRP+J2"); plt.axis('equal'); plt.grid(True); plt.legend(); plt.title("SRP & J2 drift (half period)"); plt.tight_layout()
