...
def propagate_simple(state, dt, steps, mu, extra):
    s=state.copy()
    for _ in range(steps):
        f,_=full_dynamics_with_jac(s, mu, extra)
        s[:3]+=s[3:]*dt+0.5*f*dt*dt; s[3:]+=f*dt
    return s
...
def nsga2(objective, bounds, pop=40, gen=25, pc=0.9, pm=0.15):
    D=len(bounds)
    ...
    for g in range(gen):
        ...
        for fr in fronts:
            if len(X_new)+len(fr)<=len(X):
                ...
                for k in order:
                    if len(X_new) < len(X): X_new.append(Z[fr[k]]); F_new.append(G[fr[k]])
                    else: break
                break
        X=np.array(X_new); F=np.array(F_new)
        hv_hist.append( hypervolume(G[fronts[0]]) )
    fronts,_=fast_nondominated_sort(F)
    return X[fronts[0]], F[fronts[0]], np.array(hv_hist)

def demo_objective_builder(mu, extra, start_state, target_state, T_bounds):
    def objective(y):
        t1=y[0]; dv1=y[1:4]; coast=y[4]; dv2=y[5:8]
        t1=np.clip(t1, T_bounds[0], T_bounds[1]); coast=max(0.05, min(coast, T_bounds[1]-t1))
        dt=0.02; steps1=max(1,int(t1/dt))
        s1=propagate_simple(start_state, dt, steps1, mu, extra); s1[3:]+=dv1
        steps2=max(1,int(coast/dt)); s2=propagate_simple(s1, dt, steps2, mu, extra); s2[3:]+=dv2
        Ttot=t1+coast+0.5; steps3=max(1,int((Ttot-(t1+coast))/dt)); sf=propagate_simple(s2, dt, steps3, mu, extra)
        dV=np.linalg.norm(dv1)+np.linalg.norm(dv2); tof=Ttot; robust=np.linalg.norm(sf[:3]-target_state[:3])
        return np.array([dV, tof, robust])
    return objective
