...
def propagate_with_stm(x0, T, mu=0.0121505856, extra=None, h0=1e-3, atol=1e-10, rtol=1e-9, with_stm=True):
    ...
    y = np.zeros(6 + (36 if with_stm else 0))
    y[:6] = x0.copy()
    if with_stm:
        y[6:] = np.eye(6).reshape(-1)
    t = 0.0; h = h0
    traj = [y.copy()]; ts=[0.0]
    while t < T - 1e-12:
        if t + h > T: h = T - t
        ...
        if err < tol or h<1e-8:
            t += h; y = y_new
            traj.append(y.copy()); ts.append(t)
        s = 2.0 if err==0 else 0.9*(tol/err)**0.25
        s = min(2.0, max(0.2, s)); h *= s
    return np.array(ts), np.array(traj)

def monodromy(x0, T, mu, extra):
    ts, Y = propagate_with_stm(x0, T, mu, extra, h0=1e-3, with_stm=True)
